  ...
  return (
    <div style={S.hub}>
      <div style={{ ...S.userBar, animation: "fade-in 0.4s ease" }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 700, color: "#eae2d6" }}>{user.displayName} <span style={{ color: "#00b894", fontSize: 11 }}>({role})</span></div>{oN && <div style={{ fontSize: 11, color: "#7c8a6d" }}>con {oN} 💜</div>}</div>
      ...
      <div style={{ ...S.annivBanner, animation: "fade-in 0.5s ease 0.1s both" }}>
        {!anniversary || editingDate ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            ...
            <div style={S.annivCd}><div style={S.annivBox}><div style={S.annivNum}>{ai.dM === 0 ? "🎉" : ai.dM}</div><div style={S.annivLbl}>{ai.dM === 0 ? "Mesiversario!" : `giorn${ai.dM !== 1 ? "i" : "o"} al mesiversario`}</div></div><div style={S.annivDiv} /><div style={S.annivBox}><div style={{ ...S.annivNum, color: "#f0a500" }}>{ai.dA === 0 ? "🎉" : ai.dA}</div><div style={S.annivLbl}>{ai.dA === 0 ? "Anniversario!" : `giorn${ai.dA !== 1 ? "i" : "o"} all'anniversario`}</div></div></div>
          </div>
        )}
      </div>
      ...
      <style>{`
        ...
        @keyframes orbit-glow { 0%,100% { box-shadow: 0 0 30px rgba(240,165,0,0.3); } 50% { box-shadow: 0 0 50px rgba(240,165,0,0.3); } }
      `}</style>
...
function Gusti({ data, save, role }) {
  ...
  const renderList = (items, canDelete, canLike) => {
    ...
    return GUSTI_CATS.map(gc => {
      ...
      return (
        <div key={gc.id}>
          <h3 style={{ fontSize: 13, color: "#7c8a6d", margin: "10px 0 6px" }}>{gc.icon} {gc.label}</h3>
          {catItems.map((g, i) => {
            const isLiked = (g.likes||[]).includes(role);
            const isMatched = (g.likes||[]).includes("lui") && (g.likes||[]).includes("lei") || (g.owner !== role && isLiked);
            return (
              <div key={g.id} style={{ ...S.item, animation: `fade-in 0.3s ease ${i*0.04}s both`, borderLeft: isMatched ? "3px solid #f0a500" : "3px solid rgba(255,255,255,0.06)" }}>
      ...
      {tab === "miei" && (
        <div style={S.formGroup}>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {GUSTI_CATS.map(c => (
              <button key={c.id} onClick={() => setCat(c.id)} style={{ ...S.catChip, background: cat===c.id ? "rgba(240,165,0,0.2)" : "transparent", borderColor: cat===c.id ? "#f0a500" : "rgba(255,255,255,0.06)" }}>{c.icon} {c.label}</button>
            ))}
          </div>
          <div style={S.row}>
...
function Pigiami({ data, save, role }) {
  ...
  const [comodita, setComodita] = useState(5);
  const [adattabilita, setAdattabilita] = useState(5);
  const [cute, setCute] = useState(5);

  const pigiami = data.pigiami || [];
  const sorted = [...pigiami].sort((a, b) => b.avg - a.avg);
  const podium = sorted.slice(0, 3);

  const add = () => {
    const t = name.trim();
    if (!t) return;
    const avg = +((comodita + adattabilita + cute) / 3).toFixed(1);
    save({ ...data, pigiami: [...pigiami, { id: Date.now(), name: t, owner, comodita, adattabilita, cute, avg, date: new Date().toLocaleDateString("it-IT") }] });
    setName(""); setComodita(5); setAdattabilita(5); setCute(5);
  };
  ...
  const podiumColors = podium.length >= 3
    ? ["rgba(192,192,192,0.25)", "rgba(240,165,0,0.3)", "rgba(205,127,50,0.2)"]
    : podium.length === 2 ? ["rgba(192,192,192,0.25)", "rgba(240,165,0,0.3)"] : ["rgba(240,165,0,0.3)"];

  return (
    <div style={S.sec}>
      <h2 style={S.secTitle}>🛌 Classifica Pigiami</h2>
      ...
      <div style={S.formGroup}>
        ...
        {[
          { label: "😴 Comodità", val: comodita, set: setComodita },
          { label: "🔄 Adattabilità", val: adattabilita, set: setAdattabilita },
          { label: "💕 Cute", val: cute, set: setCute },
        ].map(s => (
          <div key={s.label} style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <label style={{ fontSize: 13, width: 100, color: "#eae2d6" }}>{s.label}</label>
            <input type="range" min="1" max="10" value={s.val} onChange={e => s.set(+e.target.value)} style={{ flex: 1 }} />
            <span style={S.scoreBadge}>{s.val}</span>
          </div>
        ))}
      ...
      {podium.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <h3 style={{ fontSize: 14, color: "#7c8a6d", margin: "0 0 12px", textAlign: "center" }}>🏆 Podio</h3>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 10 }}>
            {podiumOrder.map((p, i) => (
              <div key={p.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", animation: `pop-in 0.4s ease ${0.1 + i * 0.1}s both` }}>
                <div style={{ fontSize: 28, marginBottom: 4 }}>{p.owner === "lui" ? "🙋‍♂️" : "🙋‍♀️"}</div>
                ...
                <div style={{
                  width: 80, height: podiumHeights[i], borderRadius: "12px 12px 0 0",
                  background: podiumColors[i], border: "1px solid rgba(255,255,255,0.08)",
                  borderBottom: "none", display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 22, fontWeight: 900, color: "#eae2d6", marginTop: 4,
                }}>{podiumLabels[i]}</div>
              </div>
            ))}
          </div>
          <div style={{ height: 2, background: "rgba(240,165,0,0.2)", borderRadius: 1, marginTop: 0 }} />
        </div>
      )}
      ...
      {sorted.map((p, i) => (
        <div key={p.id} style={{ ...S.item, animation: `fade-in 0.3s ease ${i * 0.04}s both`, borderLeft: `3px solid ${i < 3 ? "#f0a500" : "rgba(255,255,255,0.06)"}` }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={S.itemText}>{p.name}</div>
            <div style={{ fontSize: 10, color: "#7c8a6d" }}>
              {p.owner === "lui" ? "🙋‍♂️" : "🙋‍♀️"} · 😴{p.comodita} · 🔄{p.adattabilita} · 💕{p.cute} · {p.date}
            </div>
          </div>
...
function Stats({ data }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>📊 Statistiche di Coppia</h2>
      {stats.map((s, i) => <div key={i} style={{ ...S.statRow, animation: `fade-in 0.3s ease ${i*0.04}s both` }}><span style={{ fontSize: 20 }}>{s.i}</span><div style={{ flex: 1 }}><div style={{ fontSize: 12, color: "#7c8a6d" }}>{s.l}</div><div style={{ fontSize: 16, fontWeight: 800, color: "#eae2d6" }}>{s.v}</div></div></div>)}
    </div>
  );
}
...
const S = {
  ...
  toast:{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",background:"rgba(0,184,148,0.92)",color:"#fff",padding:"10px 20px",borderRadius:12,fontSize:13,fontWeight:700,zIndex:9999,animation:"pop-in 0.3s ease",maxWidth:"90vw",textAlign:"center"},
  annivBanner:{width:"100%",maxWidth:360,padding:"14px 16px",background:"rgba(240,165,0,0.06)",border:"1px solid rgba(240,165,0,0.12)",borderRadius:16,marginBottom:16},
  ...
  tentLine:{position:"absolute",top:"50%",left:"50%",width:2,height:55,background:"linear-gradient(to bottom,#00d4a4,transparent)",transformOrigin:"top center",borderRadius:2,opacity:0.5},
  eye:{width:56,height:56,borderRadius:"50%",background:"radial-gradient(circle,#00b894 0%,#00b894 60%,#0a1f16 100%)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 30px rgba(0,184,148,0.3)",zIndex:2},
  ...
  hubSub:{margin:"0 0 16px",fontSize:13,color:"#7c8a6d"},
  ...
  card:{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"22px 12px",border:"1px solid rgba(240,165,0,0.12)",borderRadius:16,background:"rgba(255,255,255,0.03)",color:"#eae2d6",fontSize:14,fontWeight:700,cursor:"pointer",transition:"transform 0.15s"},
