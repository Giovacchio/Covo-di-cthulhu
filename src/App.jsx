...
const CATEGORIES = [
  ...
  { id: "commedia", icon: "😂", label: "Commedia", color: "#e8c170" },
  ...
  { id: "altro", icon: "🎬", label: "Altro", color: "#8b9a7e" },
];
...
export default function App() {
  ...
  if (authLoading) return <div style={S.loadWrap}><div style={{ fontSize: 60, animation: "pop-in 0.6s ease" }}>🐙</div><p style={{ color: "#8b9a7e" }}>Risvegliando il Covo...</p></div>;
  ...
  if (!user) return (
    <div style={{ ...S.authPage, animation: "fade-in 0.5s ease" }}>
      ...
      <p style={{ color: "#8b9a7e", margin: "8px 0 28px", fontSize: 14 }}>Accedi per entrare nel Covo</p>
  ...
  if (!role) return (
    <div style={{ ...S.authPage, animation: "fade-in 0.5s ease" }}>
      ...
      <p style={{ color: "#f0ebe0", margin: "12px 0 4px", fontWeight: 700 }}>Ciao {user.displayName}!</p>
      <p style={{ color: "#8b9a7e", fontSize: 13, margin: "0 0 20px" }}>Chi sei nel Covo?</p>
      <div style={{ display: "flex", gap: 16 }}>
        <button style={{ ...S.roleBtn, opacity: usersDoc?.lui && usersDoc.lui !== user.uid ? 0.3 : 1, animation: "pop-in 0.4s ease 0.1s both" }} disabled={usersDoc?.lui && usersDoc.lui !== user.uid} onClick={() => pickRole("lui")}><span style={{ fontSize: 32 }}>🙋‍♂️</span><span>Lui</span>{usersDoc?.luiName && usersDoc.lui !== user.uid && <span style={{ fontSize: 10, color: "#8b9a7e" }}>({usersDoc.luiName})</span>}</button>
        <button style={{ ...S.roleBtn, opacity: usersDoc?.lei && usersDoc.lei !== user.uid ? 0.3 : 1, animation: "pop-in 0.4s ease 0.2s both" }} disabled={usersDoc?.lei && usersDoc.lei !== user.uid} onClick={() => pickRole("lei")}><span style={{ fontSize: 32 }}>🙋‍♀️</span><span>Lei</span>{usersDoc?.leiName && usersDoc.lei !== user.uid && <span style={{ fontSize: 10, color: "#8b9a7e" }}>({usersDoc.leiName})</span>}</button>
      </div>
      <button style={{ ...S.linkBtn, marginTop: 20 }} onClick={logout}>Cambia account</button>
    </div>
  );

  if (!loaded) return <div style={S.loadWrap}><div style={{ fontSize: 60, animation: "pop-in 0.6s ease" }}>🐙</div><p style={{ color: "#8b9a7e" }}>Caricamento dati...</p></div>;
  ...
  return (
    <div style={S.hub}>
      <div style={{ ...S.userBar, animation: "fade-in 0.4s ease" }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 700, color: "#f0ebe0" }}>{user.displayName} <span style={{ color: "#2d6a4f", fontSize: 11 }}>({role})</span></div>{oN && <div style={{ fontSize: 11, color: "#8b9a7e" }}>con {oN} 💜</div>}</div>
      ...
      <div style={{ ...S.annivBanner, animation: "fade-in 0.5s ease 0.1s both" }}>
        {!anniversary || editingDate ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}><span style={{ fontSize: 13, color: "#e8c170" }}>💜 Quando è il vostro anniversario?</span><input type="date" style={{ ...S.input, textAlign: "center", maxWidth: 200 }} defaultValue={anniversary || ""} onChange={(e) => e.target.value && setAnniv(e.target.value)} /></div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <div style={{ fontSize: 11, color: "#8b9a7e", cursor: "pointer" }} onClick={() => setEditingDate(true)}>💜 Insieme da {ai.y > 0 ? `${ai.y} ann${ai.y > 1 ? "i" : "o"} e ` : ""}{ai.m} mes{ai.m !== 1 ? "i" : "e"} · ✏️</div>
            <div style={S.annivCd}><div style={S.annivBox}><div style={S.annivNum}>{ai.dM === 0 ? "🎉" : ai.dM}</div><div style={S.annivLbl}>{ai.dM === 0 ? "Mesiversario!" : `giorn${ai.dM !== 1 ? "i" : "o"} al mesiversario`}</div></div><div style={S.annivDiv} /><div style={S.annivBox}><div style={{ ...S.annivNum, color: "#e8c170" }}>{ai.dA === 0 ? "🎉" : ai.dA}</div><div style={S.annivLbl}>{ai.dA === 0 ? "Anniversario!" : `giorn${ai.dA !== 1 ? "i" : "o"} all'anniversario`}</div></div></div>
          </div>
        )}
      </div>
...
function Watched({ data, save }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>✅ Già Visti</h2>
      {data.movies.length > 0 && <><p style={{ fontSize: 12, color: "#8b9a7e", margin: 0 }}>Segna come visto:</p><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{data.movies.map(m => <button key={m} style={S.chip} onClick={() => mw(m)}>{m} ✓</button>)}</div></>}
      {data.watched.length === 0 && <p style={S.empty}>Nessun film visto ancora!</p>}
      {data.watched.map((w, i) => <div key={i} style={{ ...S.item, animation: `fade-in 0.3s ease ${i*0.04}s both` }}><span>🎬</span><span style={S.itemText}>{w.title}</span><span style={{ fontSize: 11, color: "#8b9a7e" }}>{w.date}</span><button style={S.xBtn} onClick={() => uw(w.title)}>↩</button></div>)}
    </div>
  );
}
...
function Planner({ data, save }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>🕯️ Date Night Planner</h2>
      ...
      {[...data.plans].reverse().map((p, i) => <div key={p.id} style={{ ...S.planCard, animation: `fade-in 0.3s ease ${i*0.05}s both` }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 700, color: "#e8c170" }}>📅 {p.date}{p.time?` · 🕐 ${p.time}`:""}</span><button style={S.xBtn} onClick={() => rm(p.id)}>✕</button></div>{p.movie&&<div>🎬 {p.movie}</div>}{p.activity&&<div>🎭 {p.activity}</div>}{p.place&&<div>📍 {p.place}</div>}{p.food&&<div>🍕 {p.food}</div>}{p.drink&&<div>🍷 {p.drink}</div>}{p.note&&<div style={{ fontSize: 12, color: "#8b9a7e", fontStyle: "italic" }}>"{p.note}"</div>}</div>)}
    </div>
  );
}
...
function Reviews({ data, save, role }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>📝 Recensioni</h2>
      <p style={{ fontSize: 12, color: "#8b9a7e", margin: 0 }}>Recensisci come <strong style={{ color: "#e8c170" }}>{role}</strong></p>
      <div style={S.formGroup}>
...
function Wishlist({ data, save, role }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>💫 Wishlist Condivisa</h2>
      ...
      {WISH_TYPES.map(wt => { const items = grouped[wt.id]; if (!items) return null; return <div key={wt.id}><h3 style={{ fontSize: 14, color: "#8b9a7e", margin: "8px 0 6px" }}>{wt.icon} {wt.label}</h3>{items.map((w, i) => <div key={w.id} style={{ ...S.item, animation: `fade-in 0.3s ease ${i*0.04}s both`, opacity: w.done?0.5:1 }}><button style={{ ...S.emojiBtn, fontSize: 18 }} onClick={() => toggle(w.id)}>{w.done?"✅":"⬜"}</button><div style={{ flex: 1 }}><div style={{ ...S.itemText, textDecoration: w.done?"line-through":"none" }}>{w.title}</div>{w.note&&<div style={{ fontSize: 11, color: "#8b9a7e" }}>{w.note}</div>}<div style={{ fontSize: 10, color: "#5a6a4e" }}>aggiunto da {w.addedBy==="lui"?"🙋‍♂️":"🙋‍♀️"} · {w.date}</div></div><button style={S.xBtn} onClick={() => rm(w.id)}>✕</button></div>)}</div>; })}
    </div>
  );
}
...
function Gusti({ data, save, role }) {
  ...
  const [tab, setTab] = useState("miei"); // miei | altri | match
  ...
  const add = () => {
    const t = inp.trim();
    if (!t) return;
    save({ ...data, gusti: [...gusti, { id: Date.now(), text: t, cat, owner: role, likes: [], date: new Date().toLocaleDateString("it-IT") }] });
    setInp("");
  };
  const rm = (id) => save({ ...data, gusti: gusti.filter(g => g.id !== id) });
  const toggleLike = (id) => {
    save({ ...data, gusti: gusti.map(g => {
      if (g.id !== id) return g;
      const likes = g.likes || [];
      return { ...g, likes: likes.includes(role) ? likes.filter(l => l !== role) : [...likes, role] };
    })});
  };

  const miei = gusti.filter(g => g.owner === role);
  const altri = gusti.filter(g => g.owner === otherRole);
  ...
  const renderList = (items, canDelete, canLike) => {
    const grouped = {};
    items.forEach(g => { if (!grouped[g.cat]) grouped[g.cat] = []; grouped[g.cat].push(g); });
    ...
    return GUSTI_CATS.map(gc => {
      const catItems = grouped[gc.id];
      if (!catItems) return null;
      return (
        <div key={gc.id}>
          <h3 style={{ fontSize: 13, color: "#8b9a7e", margin: "10px 0 6px" }}>{gc.icon} {gc.label}</h3>
          {catItems.map((g, i) => {
            const isLiked = (g.likes||[]).includes(role);
            const isMatched = (g.likes||[]).includes("lui") && (g.likes||[]).includes("lei") || (g.owner !== role && isLiked);
            return (
              <div key={g.id} style={{ ...S.item, animation: `fade-in 0.3s ease ${i*0.04}s both`, borderLeft: isMatched ? "3px solid #e8c170" : "3px solid rgba(255,255,255,0.06)" }}>
                {canLike && <button onClick={() => toggleLike(g.id)} style={{ ...S.emojiBtn, fontSize: 18, background: isLiked ? "rgba(232,193,112,0.2)" : "transparent" }}>{isLiked ? "💛" : "🤍"}</button>}
                <div style={{ flex: 1 }}>
                  <div style={S.itemText}>{g.text}</div>
                  <div style={{ fontSize: 10, color: "#7a8a6e" }}>
                    {g.owner === "lui" ? "🙋‍♂️" : "🙋‍♀️"} · {g.date}
                    {isMatched && " · 💛 Match!"}
                  </div>
                </div>
  ...
  return (
    <div style={S.sec}>
      ...
      <div style={S.gustiTabs}>
        ...
        <button onClick={() => setTab("altri")} style={{ ...S.gustiTab, ...(tab === "altri" ? S.gustiTabActive : {}) }}>{otherRole === "lui" ? "🙋‍♂️" : "🙋‍♀️"} {otherRole === "lui" ? "Suoi" : "Suoi"} ({altri.length})</button>
        <button onClick={() => setTab("match")} style={{ ...S.gustiTab, ...(tab === "match" ? S.gustiTabActive : {}) }}>💛 Match ({matched.length})</button>
      </div>

      {/* Add form - only on "miei" tab */}
      {tab === "miei" && (
        <div style={S.formGroup}>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {GUSTI_CATS.map(c => (
              <button key={c.id} onClick={() => setCat(c.id)} style={{ ...S.catChip, background: cat===c.id ? "rgba(232,193,112,0.2)" : "transparent", borderColor: cat===c.id ? "#e8c170" : "rgba(255,255,255,0.06)" }}>{c.icon} {c.label}</button>
            ))}
          </div>
          <div style={S.row}>
            <input style={S.input} value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key==="Enter"&&add()} placeholder="Cosa ti piace? Es: Sushi, Rock, Hiking..." />
            <button style={S.addBtn} onClick={add}>+</button>
          </div>
        </div>
      )}

      {tab === "altri" && <p style={{ fontSize: 12, color: "#8b9a7e", margin: 0 }}>Tocca 🤍 per dire che piace anche a te!</p>}
      {tab === "match" && <p style={{ fontSize: 12, color: "#e8c170", margin: 0 }}>Le cose che piacciono a entrambi 💛</p>}
...
function Stats({ data }) {
  ...
  return (
    <div style={S.sec}><h2 style={S.secTitle}>📊 Statistiche di Coppia</h2>
      {stats.map((s, i) => <div key={i} style={{ ...S.statRow, animation: `fade-in 0.3s ease ${i*0.04}s both` }}><span style={{ fontSize: 20 }}>{s.i}</span><div style={{ flex: 1 }}><div style={{ fontSize: 12, color: "#8b9a7e" }}>{s.l}</div><div style={{ fontSize: 16, fontWeight: 800, color: "#f0ebe0" }}>{s.v}</div></div></div>)}
    </div>
  );
}
...
const S = {
  ...
  googleBtn:{display:"flex",alignItems:"center",padding:"12px 28px",borderRadius:12,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.06)",color:"#f0ebe0",fontSize:15,fontWeight:700,cursor:"pointer"},
  roleBtn:{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"20px 28px",borderRadius:16,border:"1px solid rgba(45,106,79,0.2)",background:"rgba(255,255,255,0.04)",color:"#f0ebe0",fontSize:15,fontWeight:700,cursor:"pointer"},
  linkBtn:{border:"none",background:"transparent",color:"#8b9a7e",fontSize:12,cursor:"pointer",textDecoration:"underline"},
  ...
  page:{fontFamily:"'Nunito',sans-serif",maxWidth:440,margin:"0 auto",padding:16,minHeight:"100vh",background:"linear-gradient(170deg,#0a1f16 0%,#0f2e1f 40%,#132e1a 100%)",color:"#f0ebe0"},
  hub:{fontFamily:"'Nunito',sans-serif",maxWidth:440,margin:"0 auto",padding:"24px 20px 20px",minHeight:"100vh",background:"linear-gradient(170deg,#0a1f16 0%,#0f2e1f 40%,#132e1a 100%)",color:"#f0ebe0",display:"flex",flexDirection:"column",alignItems:"center"},
  toast:{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",background:"rgba(45,106,79,0.92)",color:"#fff",padding:"10px 20px",borderRadius:12,fontSize:13,fontWeight:700,zIndex:9999,animation:"pop-in 0.3s ease",maxWidth:"90vw",textAlign:"center"},
  annivBanner:{width:"100%",maxWidth:360,padding:"14px 16px",background:"rgba(232,193,112,0.06)",border:"1px solid rgba(232,193,112,0.12)",borderRadius:16,marginBottom:16},
  ...
  annivNum:{fontSize:28,fontWeight:900,color:"#e8c170",lineHeight:1},
  annivLbl:{fontSize:10,color:"#8b9a7e",textAlign:"center"},
  ...
  hubTitle:{margin:"0 0 4px",fontSize:30,fontWeight:900,background:"linear-gradient(135deg,#2d6a4f,#e8c170)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:-0.5},
  hubSub:{margin:"0 0 16px",fontSize:13,color:"#8b9a7e"},
  suggBtn:{width:"100%",maxWidth:360,padding:"12px 0",border:"2px dashed rgba(232,193,112,0.3)",borderRadius:14,background:"rgba(232,193,112,0.06)",color:"#e8c170",fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:12},
  suggCard:{width:"100%",maxWidth:360,padding:16,background:"rgba(232,193,112,0.08)",border:"1px solid rgba(232,193,112,0.2)",borderRadius:14,marginBottom:16,fontSize:14},
  grid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,width:"100%",maxWidth:360},
  card:{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"22px 12px",border:"1px solid rgba(232,193,112,0.12)",borderRadius:16,background:"rgba(255,255,255,0.03)",color:"#f0ebe0",fontSize:14,fontWeight:700,cursor:"pointer",transition:"transform 0.15s"},
  cardIcon:{fontSize:30},cardLbl:{fontSize:13},
  footer:{marginTop:32,fontSize:10,color:"#3a4a2e",textAlign:"center",fontStyle:"italic"},
  backBtn:{border:"none",background:"rgba(232,193,112,0.12)",color:"#e8c170",padding:"8px 16px",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:16},
  sec:{display:"flex",flexDirection:"column",gap:12},
  secTitle:{fontSize:20,fontWeight:800,margin:"0 0 4px",color:"#e8c170"},
  row:{display:"flex",gap:8},
  input:{flex:1,padding:"11px 14px",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,background:"rgba(255,255,255,0.05)",color:"#f0ebe0",fontSize:14,outline:"none"},
  addBtn:{width:44,border:"none",borderRadius:10,background:"linear-gradient(135deg,#2d6a4f,#40916c)",color:"#fff",fontSize:20,fontWeight:700,cursor:"pointer"},
  bigBtn:{padding:"13px 0",border:"none",borderRadius:12,background:"linear-gradient(135deg,#2d6a4f,#e8c170)",color:"#0a1f16",fontSize:15,fontWeight:800,cursor:"pointer"},
  empty:{textAlign:"center",color:"#5a6a4e",fontSize:13,padding:28},
  ...
  itemText:{flex:1,fontSize:14,fontWeight:600,color:"#f0ebe0"},
  ...
  count:{textAlign:"center",fontSize:11,color:"#5a6a4e"},
  ...
  badge:{fontSize:10,fontWeight:700,background:"rgba(232,193,112,0.15)",color:"#e8c170",padding:"2px 8px",borderRadius:16},
  vBtn:{flex:1,padding:"9px 0",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,background:"rgba(255,255,255,0.03)",color:"#f0ebe0",fontSize:12,fontWeight:700,cursor:"pointer"},
  rstBtn:{width:36,border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,background:"transparent",color:"#8b9a7e",fontSize:15,cursor:"pointer"},
  ...
  catChip:{padding:"5px 10px",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,color:"#f0ebe0",fontSize:11,fontWeight:600,cursor:"pointer",background:"transparent"},
  ...
  scoreBadge:{fontSize:14,fontWeight:800,background:"rgba(232,193,112,0.15)",color:"#e8c170",padding:"3px 10px",borderRadius:8,minWidth:28,textAlign:"center"},
  gustiTabs:{display:"flex",gap:4,background:"rgba(255,255,255,0.03)",borderRadius:12,padding:4},
  gustiTab:{flex:1,padding:"10px 0",border:"none",borderRadius:10,background:"transparent",color:"#8b9a7e",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .2s"},
  gustiTabActive:{background:"rgba(232,193,112,0.15)",color:"#e8c170"},
